...
void TestMap::spatialDbCowTest()
{
    ...
    // 4. Test `updateBounds` interaction with COW
    {
        ...
        db2.add(RoomId{31},
                Coordinate{0,
                           0,
                           0}); // Triggers COW for db2.m_unique, sets db2.m_needsBoundsUpdate = true (implicitly by add)
        QVERIFY(
            db2.needsBoundsUpdate()); // add should have either updated bounds or marked for update
