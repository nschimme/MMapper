...
public:
    ...
    NODISCARD const AreaInfo &findGlobalArea() const { return m_global; } // Now returns ref
    NODISCARD const LocalAreaInfo &findNonGlobalArea(const RoomArea &area) const; // Now returns ref
    ...
    using FindResultVariant = std::variant<const AreaInfo *, const LocalAreaInfo *, std::nullptr_t>;
