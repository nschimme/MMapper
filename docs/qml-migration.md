...
| File I/O (open/save/save-as) | **degraded** | `file.open`/`file.save`/`file.save-as` are live: `mapstorage/MapLoadHelper.h` extracts `MainWindow`'s format-detection + background load/save logic (the widget-free half of the old load path) so `QmlShellWindow::loadFile()`/`saveMapFile()` can run it through the same `async_tasks` engine `MainWindow` uses, minus the `QProgressDialog` (the Tasks panel/`tasksModel` is this shell's load/save progress UI instead, since it already polls that same engine). `new`/`merge`/`reload`/export-as-BaseMap/XML/Web/MMP stay `**absent**` (registered disabled) -- only plain-MM2 save is wired, and there's no "discard unsaved changes?" prompt (`MainWindow::maybeSave()`) before `file.open` replaces the current map. Owner: a future commit for the remaining file commands + a maybeSave-equivalent confirmation. |
