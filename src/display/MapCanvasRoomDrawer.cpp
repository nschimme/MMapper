...
NODISCARD static UniqueMeshVector createSortedTexturedMeshes(std::string_view what,
                                                             OpenGL &gl,
...
#undef EMIT
        ...
        result_meshes.emplace_back(gl.createTexturedQuadBatch(verts, rtex.pos.array));
    };
...
NODISCARD static UniqueMeshVector createSortedColoredTexturedMeshes(
    std::string_view what, OpenGL &gl, const ColoredRoomTexVector &textures)
{
    if (textures.empty()) {
        return UniqueMeshVector{};
    }
...
#undef EMIT
        ...
        result_meshes.emplace_back(gl.createColoredTexturedQuadBatch(verts, rtex.pos.array));
    };
