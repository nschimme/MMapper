...
FutureSharedMapBatchFinisher MapData::generateBatches(const mctp::MapCanvasTexturesProxy &textures)
{
    ...
    // TODO: The LayerMeshes generated by this finisher should populate LayerMeshes::m_area.
    // This might require changes in MapCanvasRoomDrawer or how generateMapDataFinisher works.
...
RoomIdSet MapData::getRoomIdsInBoundingBox(const Bounds& bounds, int layer) const
{
    return getCurrentMap().getWorld().getSpatialDb().getRoomIdsInBounds(bounds, layer);
}

std::optional<RoomArea> MapData::getRoomArea(RoomId id) const
{
    if (const auto roomHandle = findRoomHandle(id)) {
        return roomHandle.getArea();
    }
    return std::nullopt;
}
...
{
    // The LayerMeshes generated by this new finisher *must* have their LayerMeshes::m_area field populated.

    // Prepare areaData map for the finisher
    ...
    for (const RoomArea& area : areasToGenerate) {
        ...
        if (currentAreaInfo) {
            ...
            areaData[area] = RoomIdSet{};
        }
    }
    // This now calls the function defined in MapCanvasRoomDrawer.h/cpp
    return ::generateRoomAreaDataFinisher(textures, getCurrentMap(), areasToGenerate, areaData);
}
