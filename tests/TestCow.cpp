...
void TestCow::testReadOnlySharing() {
    ...
    QVERIFY(c1.get().use_count() > 1); // Both c1 and c2 should share it, plus raw_room_data if still in scope
...
void TestCow::testLazyCopyOnWrite() {
    ...
    QVERIFY(static_cast<const void*>(writable_room_ptr.get()) != static_cast<const void*>(initial_addr));
    QCOMPARE(writable_room_ptr->fields.Name.toStdStringUtf8(), "Initial"); // Content should be copied

    // Modify the copy
    writable_room_ptr->fields.Name = makeRoomName("Modified");
    QCOMPARE(c1.get()->fields.Name.toStdStringUtf8(), "Modified"); // c1.get() should now point to the modified copy
    QCOMPARE(initial_raw_room->fields.Name.toStdStringUtf8(), "Initial"); // Original const shared_ptr data unchanged
}
...
void TestCow::testMutationIsolation() {
    ...
    QVERIFY(static_cast<const void*>(c1.get().get()) != static_cast<const void*>(c2.get().get())); // c1 should have a new copy
...
void TestCow::testFinalize() {
    ...
    QCOMPARE(const_addr_after_finalize, writable_addr1); // Should be same address, just const_cast by finalize
...
void TestCow::testWorldCopyCOW() {
    ...
    world1_for_copy.applyOne(pc, Change{room_change_types::ModifyRoomFlags{room_id_in_w1, makeRoomName("OriginalName"), FlagModifyModeEnum::ASSIGN}});
    ...
    world2.applyOne(pc, Change{room_change_types::ModifyRoomFlags{room_id_in_w1, makeRoomName("ModifiedInW2"), FlagModifyModeEnum::ASSIGN}});
    ...
    QVERIFY(static_cast<const void*>(r1_w1_ptr.get()) != static_cast<const void*>(r1_w2_modified_ptr.get()));
...
void TestCow::testWorldApplyChangeCOW() {
    ...
    world.applyOne(pc, Change{room_change_types::ModifyRoomFlags{room_id, makeRoomName("Initial"), FlagModifyModeEnum::ASSIGN}});
    ...
    world.applyOne(pc, Change{room_change_types::ModifyRoomFlags{room_id, makeRoomName("ChangedName"), FlagModifyModeEnum::ASSIGN}});
