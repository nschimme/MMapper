...
#include <QObject>
#include <QString>
// #include <QtCore> // QObject related, might not be needed if nothing else from QtCore is used. Let's keep for now.
#include <QtCore>

// Removed: class RoomRecipient;
struct RoomId;
class MapFrontend;
...
class RoomSignalHandler final // Removed QObject inheritance and NODISCARD_QOBJECT
{
    // Removed Q_OBJECT
...
private:
    MapFrontend &m_map;
    RoomIdSet owners;
    std::map<RoomId, std::set<PathMachine::PathProcessor *>> lockers; // CHANGED
    std::map<RoomId, int> holdCount;

public:
    RoomSignalHandler() = delete;
    explicit RoomSignalHandler(MapFrontend &map); // Removed QObject *parent
    /* receiving from our clients: */
    // hold the room, we don't know yet what to do, overrides release, re-caches if room was un-cached
    void hold(RoomId room, PathMachine::PathProcessor *locker); // CHANGED
