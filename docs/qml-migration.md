...
| File I/O (open/save/save-as/new/merge/reload/exports) | live | All 10 file commands are wired: `mapstorage/MapLoadHelper.h` extracts `MainWindow`'s format-detection + background load/merge/save logic (the widget-free half of the old load/merge path -- `maploadhelper::loadMapData()`/`mergeMapData()`) so `QmlShellWindow::loadFile()`/`mergeFile()`/`saveMapFile()` can run it through the same `async_tasks` engine `MainWindow` uses. `saveMapFile()` now takes `SaveModeEnum`/`SaveFormatEnum` (mirroring `MainWindow::saveFile()`'s per-format storage selection), so the 4 `file.export.*` commands (BaseMap/XML/Web/MMP) share it with plain save/save-as. `file.new`/`file.reload` mirror `MainWindow::slot_newFile()`/`slot_reload()`. Deviation: `onSuccessfulSave()`'s "Autoload this map?" follow-up prompt isn't ported. |
| Unsaved-changes guard (`maybeSave`) | live | `QmlShellWindow::maybeSave()` ports `MainWindow::maybeSave()` verbatim (same `QMessageBox::Save`/`Discard`/`Cancel` prompt + `MapData::describeChanges()` text), invoked before `file.open`/`file.new`/`file.reload` (matching which ops `MainWindow` guards -- `file.merge` is intentionally unguarded, same as Shell A, since merging doesn't discard anything) and on window close via `confirmClose()` (`Q_INVOKABLE`, called from `MainShell.qml`'s `Window.onClosing`; `file.exit` now routes through `window->close()` instead of `qApp->quit()` specifically so this fires). Deviation: `confirmClose()` refuses to close outright while any IO task is running rather than `MainWindow::closeEvent()`'s cancel-and-wait-then-quit dance. |
| Async load/merge/save progress popup | live | A modal `QQC2.Popup` in `MainShell.qml` (`ioProgressPopup`), driven by `IoTaskController` (`mainwindow/IoTaskController.h`) via the `ioTask` context property: label + `ProgressBar` + a Cancel button enabled only for cancelable tasks (loads/merges; saves/exports forbid cancel, mirroring `MainWindow::AsyncIO`'s `QProgressDialog` -- `createNewProgressDialog()`'s disabled-Cancel-button case). `QmlShellWindow` polls the running task's `ProgressCounter` on a 25ms `QTimer` (`tickIoTaskProgress()`, mirroring `AsyncIO::updateStatus()`) and auto-dismisses on completion/failure/cancel (`endIoTaskProgress()`). Previously the (hidden-by-default) Tasks panel was the only load/save feedback; it remains a secondary view of the same `async_tasks` registry. |
| Startup autoload (`--qml-shell` loads the configured/asset map like Shell A) | live | `main.cpp`'s `tryLoad()`/`tryAutoLoadMap()` are now templated on the shell type (`MainWindow`/`QmlShellWindow`, both exposing `loadFile(std::shared_ptr<MapSource>)`) and called for Shell B right after construction; the Wasm network-sideload branch stays `MainWindow`-only (`if constexpr`) since Shell B is never constructed under `Q_OS_WASM` anyway. |
| Map-view splash overlay (`MapView.qml`) | live | `QmlShellWindow` drives a `mapLoaded` context property that mirrors both of `MapWindow::hideSplashImage()`'s widget-side triggers: hidden once a load attempt completes (success or failure, via `QmlShellWindow::hideSplash()`, mirroring `mainwindow/utils.cpp`'s `CanvasDisabler` destructor) or once the map is modified (`QmlShellWindow::updateMapModifiedState()`, mirroring `MainWindow::setMapModified(true)`); stays visible for a never-loaded/empty map, same as Shell A. |
| Proxy/telnet/MUD-client backend + listener services | live | `QmlShellWindow::startServices()` constructs the same `ConnectionListener`/`Proxy` graph `MainWindow::startServices()` does and installs a `ClientTelnetBackend` on `ClientController`, so the Client panel's Play button drives a real telnet connection. `Proxy`/`ConnectionListener` no longer depend on the concrete `MainWindow` type or a widget `MapCanvas&`: they now take a `ProxyHostApi&` (`src/proxy/ProxyHostApi.h`, implemented by both `MainWindow` and `QmlShellWindow`) and a `MapCanvasCore&` (`MapCanvas::getCore()`), which is what let this be wired into Shell B at all. Bind failure falls back to `QMessageBox::critical` (reused unchanged, same as Shell A -- `QApplication` is retained) plus a `statusText` update. Deviation from Shell A: no startup auto-update-check call in `startServices()` (the existing `help.check-for-update` command still works); `RemoteEdit`'s edit-session windows have no Shell-B-side wrapper UI beyond what `Proxy::allocRemoteEdit()` itself opens. |
| Path machine feed to status bar / canvas | live | `Mmapper2PathMachine` is constructed with real deps; `QmlShellWindow::wirePathMachine()` mirrors `MainWindow::wireConnections()`'s path-machine block: `sig_playerMoved` -> `MapCanvasCore::slot_moveMarker` + description/audio refresh, `MapData::sig_onPositionChange` -> `onPositionChange()` + description/audio refresh, `MapData::sig_onForcedPositionChange` -> `MapCanvasCore::slot_onForcedPositionChange`, `sig_state` -> the `pathMachineStatus` context property (see above). `Mmapper2PathMachine::onMapLoaded()` is called from `onSuccessfulLoad()`. |
| Description panel live feed | live | Fed by the same `wirePathMachine()`/`onSuccessfulLoad()` calls to `DescriptionAdapter::updateRoom()` MainWindow uses; no longer depends on a telnet backend being absent. |
| `room.goto-selected` / `room.force-update-selected` | live | Ported from `MainWindow`'s `gotoRoomAct`/`slot_forceMapperToRoom()`; enabled only for a single-room selection (`force-update` additionally requires `Mmapper2PathMachine::hasLastEvent()`), tracked in `QmlShellWindow::wireSelectionCommands()`'s `sig_newRoomSelection` handler. |
