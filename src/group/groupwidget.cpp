...
void GroupModel::updateCharacter(const SharedGroupChar& updatedCharacter)
{
    ...
    if (index != -1) {
        ...
        // The logic for re-sorting/moving the character if its NPC status changed
        // (which involved calling this->setCharacters()) has been removed as per the task.
        // The character will remain in its current row, and only its data will be updated.
    } else {
