...
void Experimenting::augmentPath(const std::shared_ptr<Path> &path, const RoomHandle &room)
{
    ...
    const auto working = p.fork(room, c, m_params, this->getSharedPtrFromThis(), m_dirCode);
    if (m_best == nullptr) {
        m_best = working;
...
std::shared_ptr<PathList> Experimenting::evaluate(ChangeList &changes)
{
    ...
    if (m_best != nullptr) {
        if (m_second == nullptr || m_best->getProb() > m_second->getProb() * m_params.acceptBestRelative
            || m_best->getProb() > m_second->getProb() + m_params.acceptBestAbsolute) {
            for (auto &path_ptr : *m_paths) {
                path_ptr->deny(changes);
            }
            m_paths->clear();
