...
private:
    void virt_setUniforms(const glm::mat4 &mvp, const GLRenderState &renderState) final
    {
        setColor("uColor", renderState.uniforms.color);
        setMatrix("uMVP", mvp);
    }
};
...
private:
    void virt_setUniforms(const glm::mat4 &mvp, const GLRenderState &renderState) final
    {
        assert(renderState.uniforms.textures[0] != INVALID_MM_TEXTURE_ID);
