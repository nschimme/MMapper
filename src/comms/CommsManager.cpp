...
CommsManager::CommsManager(GameObserver &observer, QObject *parent)
    : QObject(parent)
{
    observer.sig2_sentToUserGmcp.connect(m_lifetime, [this](const GmcpMessage &gmcp) {
        parseGmcpInput(gmcp);
    });
    observer.sig2_rawGameText.connect(m_lifetime, [this](const QString &text) {
        parseRawGameText(text);
    });
}

CommsManager::~CommsManager() = default;

void CommsManager::parseGmcpInput(const GmcpMessage &msg)
{
...
void CommsManager::parseRawGameText(const QString &rawText)
{
