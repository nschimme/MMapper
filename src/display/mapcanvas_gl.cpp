...
void MapCanvas::finishPendingMapBatches()
{
    ...
    for (auto it = m_batches.m_areaRemeshCookies.begin(); it != m_batches.m_areaRemeshCookies.end(); /* manual increment below */) {
        ...
        } else if (!cookie.isPending() && !cookie.isReady()) { // Cleanup stale ignored cookies (e.g. setIgnored then processed)
             it = m_batches.m_areaRemeshCookies.erase(it);
        }
        else {
            ++it; 
...
void MapCanvas::paintMap()
{
    ...
    if (m_batches.m_areaMapBatches.empty()) {
        const QString msg = any_area_pending ? "Please wait... the map isn't ready yet." : "No map data to display."; // Adjusted message
...
void MapCanvas::renderMapBatches()
{
    ...
    auto renderSingleMapBatch =
        [&](const MapBatches& currentMapBatches, const std::string& areaNameHint = "") { // Added areaNameHint for logging
    ...
    } else if (renderedSomething && areaIsActuallyPending) { // Rendered something, but still area work pending
        setAnimating(true); 
        update();
