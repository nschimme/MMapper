...
private:
    ...
    // Compact-layout only (see setCompactLayout()): a row of large
    ...
    // while compact, and the last entry of the map's context menu, so that
    // a touch user (no hover to "peek" a hidden menu bar with) can always
    // reach them by a long-press on the map. A QAction can sit in several
    // widgets at once, so the menus stay in the menu bar as well.
