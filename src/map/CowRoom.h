...
public:
    ...
    CowRoom()
        : m_room_variant(std::make_shared<RawRoom>())
    {}

    // Constructor: Initializes with a read-only RawRoom
    CowRoom(std::shared_ptr<const RawRoom> room)
        : m_room_variant(room)
    {}

    // Constructor: Initializes with a writable RawRoom (e.g., a new room)
    CowRoom(std::shared_ptr<RawRoom> room)
        : m_room_variant(room)
    {}
    ...
    std::shared_ptr<RawRoom> getMutable()
    {
        if (std::holds_alternative<std::shared_ptr<const RawRoom>>(m_room_variant)) {
            ...
            auto writable_ptr = std::make_shared<RawRoom>(
                *const_ptr);               // Assumes RawRoom has a copy constructor
            m_room_variant = writable_ptr; // Update this instance to point to the new copy
            return writable_ptr;
        } else { // Already holds std::shared_ptr<RawRoom>
            std::shared_ptr<RawRoom> &current_writable_ptr = std::get<std::shared_ptr<RawRoom>>(
                m_room_variant);
    ...
    void finalize()
    {
        if (std::holds_alternative<std::shared_ptr<RawRoom>>(m_room_variant)) {
            ...
            m_room_variant = std::static_pointer_cast<const RawRoom>(
                std::get<std::shared_ptr<RawRoom>>(m_room_variant));
        }
