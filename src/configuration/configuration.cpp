...
void Configuration::CanvasSettings::read(QSettings &conf)
{
    ...
    conf.beginGroup("WeatherEffects");
    advanced.weatherEffects.showFog.set(conf.value("showFog", true).toBool());
    advanced.weatherEffects.showRain.set(conf.value("showRain", true).toBool());
    advanced.weatherEffects.showSnow.set(conf.value("showSnow", true).toBool());
    advanced.weatherEffects.showTimeOfDay.set(conf.value("showTimeOfDay", true).toBool());
    conf.endGroup();
}
...
void Configuration::CanvasSettings::write(QSettings &conf) const
{
    ...
    conf.beginGroup("WeatherEffects");
    conf.setValue("showFog", advanced.weatherEffects.showFog.get());
    conf.setValue("showRain", advanced.weatherEffects.showRain.get());
    conf.setValue("showSnow", advanced.weatherEffects.showSnow.get());
    conf.setValue("showTimeOfDay", advanced.weatherEffects.showTimeOfDay.get());
    conf.endGroup();
}
...
Configuration::CanvasSettings::Advanced::WeatherEffects::WeatherEffects() = default;

void Configuration::CanvasSettings::Advanced::WeatherEffects::registerChangeCallback(
    const ChangeMonitor::Lifetime &lifetime, const ChangeMonitor::Function &callback)
{
    /* copied for each; consider changing this to be std::shared_ptr<Function> */
    showFog.registerChangeCallback(lifetime, callback);
    showRain.registerChangeCallback(lifetime, callback);
    showSnow.registerChangeCallback(lifetime, callback);
    showTimeOfDay.registerChangeCallback(lifetime, callback);
}
