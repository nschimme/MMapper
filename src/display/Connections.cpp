...
void MapCanvas::paintSelectedConnection()
{
    ...
    {
        std::vector<Legacy::LineVert> verts;
        verts.emplace_back(pos1, pos2, Colors::red, CONNECTION_LINE_WIDTH);
...
void ConnectionDrawer::ConnectionFakeGL::drawLineStrip(const std::vector<glm::vec3> &points)
{
    ...
    for (size_t i = 1; i < size; ++i) {
        ...
        if (mmgl::isNearZero(segment)) {
            verts.emplace_back(start_v, end_v, current_segment_color, CONNECTION_LINE_WIDTH);
            continue;
        }
        ...
        glm::vec3 line_start_v = start_v;
        glm::vec3 line_end_v = end_v;
        const glm::vec3 segment_dir = glm::normalize(segment);

        if (i == 1) {
            line_start_v = start_v - segment_dir * extension;
        }
        if (i == size - 1) {
            line_end_v = end_v + segment_dir * extension;
        }

        if (!isLongLine(line_start_v, line_end_v)) {
            verts.emplace_back(line_start_v, line_end_v, current_segment_color, CONNECTION_LINE_WIDTH);
            continue;
        }

        const float len = glm::length(line_end_v - line_start_v);
        const float faintCutoff = (len > 1e-6f) ? (LONG_LINE_HALFLEN / len) : 0.5f;

        const glm::vec3 mid1 = glm::mix(line_start_v, line_end_v, faintCutoff);
        const glm::vec3 mid2 = glm::mix(line_start_v, line_end_v, 1.f - faintCutoff);
        const Color faint_color = current_segment_color.withAlpha(FAINT_CONNECTION_ALPHA);

        verts.emplace_back(line_start_v, mid1, current_segment_color, CONNECTION_LINE_WIDTH);
        verts.emplace_back(mid1, mid2, faint_color, CONNECTION_LINE_WIDTH);
        verts.emplace_back(mid2, line_end_v, current_segment_color, CONNECTION_LINE_WIDTH);
    }
}
