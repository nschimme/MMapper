...
public:
    NODISCARD static std::shared_ptr<MapSource> alloc(
        const QUrl &url, const std::optional<QByteArray> &fileContent = std::nullopt) CAN_THROW;

public:
    explicit MapSource(Badge<MapSource>, const QUrl &url, std::shared_ptr<QIODevice> device);
