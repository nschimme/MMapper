...
public:
    ...
    CopyOnWrite()
        : m_data_variant(std::make_shared<T>())
    {}

    // Returns a const shared_ptr to the data.
    std::shared_ptr<const T> get() const
    {
        return std::visit([](const auto &ptr) -> std::shared_ptr<const T> { return ptr; },
                          m_data_variant);
    }
    ...
    std::shared_ptr<T> getMutable()
    {
        if (std::holds_alternative<std::shared_ptr<const T>>(m_data_variant)) {
            ...
            if (current_writable_ptr
                && current_writable_ptr.use_count()
                       > 1) { // Added null check for current_writable_ptr
    ...
    bool isWritable() const { return std::holds_alternative<std::shared_ptr<T>>(m_data_variant); }
    ...
    bool operator!=(const CopyOnWrite<T> &other) const { return !(*this == other); }
