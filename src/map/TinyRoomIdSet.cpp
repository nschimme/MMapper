...
static void testTinyRoomIdSet_CopyOnWrite()
{
    ...
    TEST_ASSERT(checkContents(set1, std::vector<RoomId>{RoomId{1}, RoomId{2}}));
    TEST_ASSERT(checkContents(set2, std::vector<RoomId>{RoomId{1}, RoomId{2}}));

    set1.insert(RoomId{3}); // Modify set1, COW should occur
    TEST_ASSERT(checkContents(set1, std::vector<RoomId>{RoomId{1}, RoomId{2}, RoomId{3}})); // set1 is {1, 2, 3}
    TEST_ASSERT(checkContents(set2, std::vector<RoomId>{RoomId{1}, RoomId{2}}));     // set2 should remain {1, 2}
    TEST_ASSERT(set1 != set2);
    ...
    TEST_ASSERT(checkContents(set4, std::vector<RoomId>{RoomId{10}, RoomId{20}}));

    set3.erase(RoomId{10}); // Modify set3, COW should occur
    TEST_ASSERT(checkContents(set3, std::vector<RoomId>{RoomId{20}}));      // set3 is {20}
    TEST_ASSERT(checkContents(set4, std::vector<RoomId>{RoomId{10}, RoomId{20}})); // set4 should remain {10, 20}
    TEST_ASSERT(set3 != set4);
    ...
    TEST_ASSERT(checkContents(set5, std::vector<RoomId>{RoomId{200}}));
    TEST_ASSERT(checkContents(set6, std::vector<RoomId>{RoomId{100}, RoomId{200}}));
    ...
    TEST_ASSERT(checkContents(empty1, std::vector<RoomId>{RoomId{1}}));
    ...
    TEST_ASSERT(checkContents(single1, std::vector<RoomId>{RoomId{7}, RoomId{8}}));
    TEST_ASSERT(checkContents(single2, std::vector<RoomId>{RoomId{7}})); // single2 should remain {7}
}
