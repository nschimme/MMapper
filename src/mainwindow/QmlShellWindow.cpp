...
struct NODISCARD CommandSpec final
{
    ...
    const char *text = "";
    const char *shortcut = "";
    // Mirrors the QAction::setStatusTip() text MainWindow::createActions()
    // sets on the equivalent action (see mainwindow.cpp); shown by
    // MainShell.qml's toolbar ToolButtons as a hover ToolTip. Empty means
    // the widget action had no status tip either.
    const char *toolTip = "";
};
...
constexpr std::array<CommandSpec, 65> ALL_COMMAND_SPECS = {
    CommandSpec{"file.new", false, "&New", "", "Create a new file"},
    CommandSpec{"file.open", false, "&Open...", "Ctrl+O", "Open an existing file"},
    CommandSpec{"file.merge", false, "&Merge...", "", "Merge an existing file into current map"},
    CommandSpec{"file.reload", false, "&Reload", "Ctrl+R", "Reload the current map"},
    CommandSpec{"file.save", false, "&Save", "Ctrl+S", "Save the document to disk"},
    CommandSpec{"file.save-as", false, "Save &As...", "", "Save the document under a new name"},
    CommandSpec{"file.export.base-map",
                false,
                "Export MMapper2 &Base Map As...",
                "",
                "Save a copy of the map with no secrets"},
    CommandSpec{"file.export.mm2xml",
                false,
                "Export MMapper2 &XML Map As...",
                "",
                "Save a copy of the map in the MM2XML format"},
    CommandSpec{"file.export.web",
                false,
                "Export &Web Map As...",
                "",
                "Save a copy of the map for webclients"},
    CommandSpec{"file.export.mmp",
                false,
                "Export &MMP Map As...",
                "",
                "Save a copy of the map in the MMP format"},
    CommandSpec{"file.exit", false, "E&xit", "Ctrl+Q", "Exit the application"},
    CommandSpec{"edit.undo", false, "&Undo", "", "Undo the last action"},
    CommandSpec{"edit.redo", false, "&Redo", "", "Redo the last undone action"},
    CommandSpec{"edit.preferences", false, "&Preferences", "Ctrl+P", "MMapper preferences"},
    CommandSpec{"view.zoom-in", false, "Zoom In", "Ctrl++", "Zooms In current map"},
    CommandSpec{"view.zoom-out", false, "Zoom Out", "Ctrl+-", "Zooms Out current map"},
    CommandSpec{"view.zoom-reset", false, "Zoom Reset", "Ctrl+0", "Zoom to original size"},
    ...
    CommandSpec{"layer.up", false, "Layer Up", "Ctrl+Tab", "Layer Up"},
    CommandSpec{"layer.down", false, "Layer Down", "Ctrl+Shift+Tab", "Layer Down"},
    CommandSpec{"layer.reset", false, "Layer Reset", "", "Layer Reset"},
    CommandSpec{"mouse-mode.move", true, "Move map", "", "Move Map"},
    CommandSpec{"mouse-mode.room-raypick", true, "Ray-pick Rooms", "", "Ray-pick Rooms"},
    CommandSpec{"mouse-mode.room-select", true, "Select Rooms", "", "Select Rooms"},
    CommandSpec{"mouse-mode.connection-select", true, "Select Connection", "", "Select Connection"},
    CommandSpec{"mouse-mode.create-room", true, "Create New Rooms", "", "Create New Rooms"},
    CommandSpec{"mouse-mode.create-connection",
                true,
                "Create New Connection",
                "",
                "Create New Connection"},
    CommandSpec{"mouse-mode.create-oneway-connection",
                true,
                "Create New Oneway Connection",
                "",
                "Create New Oneway Connection"},
    CommandSpec{"mouse-mode.infomark-select", true, "Select Markers", "", "Select Info Markers"},
    CommandSpec{"mouse-mode.create-infomark",
                true,
                "Create New Markers",
                "",
                "Create New Info Markers"},
    CommandSpec{"mapper-mode.play",
                true,
                "Switch to play mode",
                "",
                "Switch to play mode - no new rooms are created"},
    CommandSpec{"mapper-mode.map",
                true,
                "Switch to mapping mode",
                "",
                "Switch to mapping mode - new rooms are created when moving"},
    CommandSpec{"mapper-mode.offline",
                true,
                "Switch to offline emulation mode",
                "",
                "Switch to offline emulation mode - you can learn areas offline"},
    CommandSpec{"room.create", false, "Create New Room", "", "Create a new room under the cursor"},
    CommandSpec{"room.edit-selected", false, "Edit Selected Rooms", "Ctrl+E", "Edit Selected Rooms"},
    CommandSpec{"room.delete-selected", false, "Delete Selected Rooms", "", "Delete Selected Rooms"},
    CommandSpec{"room.move-up-selected",
                false,
                "Move Up Selected Rooms",
                "",
                "Move Up Selected Rooms"},
    CommandSpec{"room.move-down-selected",
                false,
                "Move Down Selected Rooms",
                "",
                "Move Down Selected Rooms"},
    CommandSpec{"room.merge-up-selected",
                false,
                "Merge Up Selected Rooms",
                "",
                "Merge Up Selected Rooms"},
    CommandSpec{"room.merge-down-selected",
                false,
                "Merge Down Selected Rooms",
                "",
                "Merge Down Selected Rooms"},
    CommandSpec{"room.connect-to-neighbours",
                false,
                "Connect room(s) to its neighbour rooms",
                "",
                "Connect room(s) to its neighbour rooms"},
    CommandSpec{"room.find", false, "&Find Rooms", "Ctrl+F", "Find matching rooms"},
    CommandSpec{"room.goto-selected", false, "Move to selected room", "", "Move to selected room"},
    CommandSpec{"room.force-update-selected",
                false,
                "Update selected room with last movement",
                "",
                "Update selected room with last movement"},
    CommandSpec{"connection.delete-selected",
                false,
                "Delete Selected Connection",
                "",
                "Delete Selected Connection"},
    CommandSpec{"infomark.edit-selected",
                true,
                "Edit Selected Markers",
                "",
                "Edit Selected Info Markers"},
    CommandSpec{"infomark.delete-selected",
                true,
                "Delete Selected Markers",
                "",
                "Delete Selected Info Markers"},
    CommandSpec{"client.launch", false, "&Launch mud client", "", "Launch the integrated mud client"},
    CommandSpec{"client.save-log",
                false,
                "Save Log as &Plain Text...",
                "",
                "Save log as plain text file"},
    CommandSpec{"client.save-log-html", false, "Save Log as &HTML...", "", "Save log as HTML file"},
    CommandSpec{"pathmachine.release-all-paths", false, "Release All Paths", "", "Release all paths"},
    CommandSpec{"world.rebuild-meshes",
                false,
                "&Rebuild World",
                "",
                "Reconstruct the world mesh to fix graphical rendering bugs"},
    CommandSpec{"help.vote",
                false,
                "V&ote for Mume",
                "",
                "Please vote for MUME on \"The Mud Connector\""},
    ...
    CommandSpec{"help.newbie",
                false,
                "&Information for Newcomers",
                "",
                "Newbie help on the MUME website"},
    CommandSpec{"help.report-issue",
                false,
                "Report an &Issue...",
                "",
                "Open the MMapper issue tracker in your browser"},
    CommandSpec{"help.about", false, "About &MMapper", "", "Show the application's About box"},
    CommandSpec{"help.about-qt", false, "About &Qt", "", "Show the Qt library's About box"},
};
...
void QmlShellWindow::registerCommands()
{
    for (const CommandSpec &spec : ALL_COMMAND_SPECS) {
        ...
        if (*spec.toolTip != '\0') {
            cmd->setToolTip(QString::fromUtf8(spec.toolTip));
        }
        if (*spec.shortcut != '\0') {
