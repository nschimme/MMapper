...
void MapCanvas::updateBatches()
{
    ...
    if (any_task_ready_to_process) {
        processCompletedRemeshes();
        // processCompletedRemeshes() is now responsible for calling update()
...
void MapCanvas::updateMapBatches()
{
    ...
    // Now, it translates to requesting remesh for all known areas.
    if (!m_opengl.isRendererInitialized()) {
        return;
    }
    qInfo() << "MapCanvas: updateMapBatches called, requesting remesh for all known areas.";
    slot_handleAreaRemesh({}); // Pass empty set to trigger remesh of all known areas

    // Original logic also cleared area batches and ignored pending area cookies.
    // slot_handleAreaRemesh should now correctly manage new tasks for all areas.
    // Existing area batches will be replaced when new ones are ready.
    // If an area had a pending cookie, slot_handleAreaRemesh would have previously
    // marked it for catch-up; now it will just log that it's pending.
    // The new tasks generated by slot_handleAreaRemesh will supersede.
...
void MapCanvas::finishPendingMapBatches()
{
    ...
    // Global Remesh Cookie and Batches are removed.
...
void MapCanvas::paintMap()
{
    ...
    // If there are no area batches and no global batch (which is now removed)
    if (m_batches.m_areaMapBatches.empty()) {
        const QString msg = any_area_pending ? "Please wait... the map isn't ready yet."
                                             : "No map data to display."; // Adjusted message
        getGLFont().renderTextCentered(msg);
        if (!any_area_pending) {
            // REVISIT: does this need a better fix?
            // any_area_pending already scheduled an update, but now we realize we need an update.
            update();
        }
        return;
    }
    ...
    if (any_area_pending) { // Changed from 'pending' to 'any_area_pending'
...
void MapCanvas::renderMapBatches()
{
    ...
    auto renderSingleMapBatch =
        [&](const MapBatches &currentMapBatches,
            ...
            BatchedMeshes &batchedMeshes = const_cast<BatchedMeshes &>(
                currentMapBatches.batchedMeshes);
            BatchedConnectionMeshes &connectionMeshes = const_cast<BatchedConnectionMeshes &>(
                currentMapBatches.connectionMeshes);
            BatchedRoomNames &roomNameBatches = const_cast<BatchedRoomNames &>(
                currentMapBatches.roomNameBatches);

            const auto drawLayer = [&](const int thisLayer,
                                       const int currentLayerParam) { // Renamed to avoid conflict
            ...
            for (const auto &pair : batchedMeshes)
                layers_to_render.insert(pair.first);
            for (const auto &pair : connectionMeshes)
                layers_to_render.insert(pair.first);
            if (wantExtraDetail && wantDoorNames) {
                for (const auto &pair : roomNameBatches)
                    layers_to_render.insert(pair.first);
            }
    ...
    if (!m_batches.m_areaMapBatches.empty()) {
        ...
        // If there are no area batches, renderedSomething will remain false.
    ...
    if (!renderedSomething && areaIsActuallyPending) { // Simplified: only check area pending
        const bool flashOn = m_batches.pendingUpdateFlashState.tick();
        if (flashOn) {
            // renderPendingUpdate(); // Removed as function is missing
            // Simple text indication can be done here if needed, e.g. using getGLFont().renderTextCentered("Updating...");
            getGLFont().renderTextCentered("Updating map areas...");
        }
        setAnimating(true); // Ensure renderLoop continues if pending
        update();
    } else if (!areaIsActuallyPending) { // Simplified: if no area is pending
        setAnimating(false);             // Nothing pending, stop animation
    } else if (renderedSomething
               && areaIsActuallyPending) { // Rendered something, but still area work pending
        setAnimating(true);
        update();
