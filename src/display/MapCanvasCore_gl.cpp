...
void MapCanvasCore::hostCleanupGL()
{
...
void MapCanvasCore::reportGLVersion()
{
    auto &gl = getOpenGL();
...
#undef X_CHECK
    ...
    logMsg("Display:", QString("%1 DPI").arg(currentDpr()).toUtf8());
}

bool MapCanvasCore::isBlacklistedDriver()
{
...
bool MapCanvasCore::hostInitializeGL()
{
    OpenGL &gl = getOpenGL();
    try {
        ...
        // to this signal, preserving today's UX exactly.
    ...
    // compile all shaders
    {
        auto &sharedFuncs = gl.getSharedFunctions(Badge<MapCanvasCore>{});
...
void MapCanvasCore::slot_onMessageLoggedDirect(const QOpenGLDebugMessage &message)
{
...
void MapCanvasCore::setMvp(const glm::mat4 &viewProj)
{
    auto &gl = getOpenGL();
...
void MapCanvasCore::setViewportAndMvp(const int width, const int height, const qreal dpr)
{
...
void MapCanvasCore::onViewProjDirty() const
{
    m_opengl.getUboManager().invalidate(Legacy::SharedVboEnum::CameraBlock);
}

void MapCanvasCore::hostResize(const int width, const int height, const qreal dpr)
{
...
void MapCanvasCore::updateBatches()
{
    updateMapBatches();
    updateInfomarkBatches();
}

void MapCanvasCore::updateMapBatches()
{
...
void MapCanvasCore::finishPendingMapBatches()
{
...
void MapCanvasCore::actuallyPaintGL()
{
    // DECL_TIMER(t, __FUNCTION__);

    // Deferred (defer-to-next-render) GL work: these historically ran outside
    // of paintGL() and relied on the host's GL context happening to still be
    // current. Applying them here, at the top of the one place that's
    // guaranteed to have a current context, makes that assumption explicit
    // and safe for any future host (e.g. a QQuickFramebufferObject item
    // rendering on a dedicated render thread).
    ...
    auto &gl = getOpenGL();
    auto &funcs = deref(gl.getSharedFunctions(Badge<MapCanvasCore>{}));
...
NODISCARD bool MapCanvasCore::Diff::isUpToDate(const Map &saved, const Map &current) const
{
...
NODISCARD bool MapCanvasCore::Diff::hasRelatedDiff(const Map &saved) const
{
    return highlight && highlight->saved.isSamePointer(saved);
}

void MapCanvasCore::Diff::cancelUpdates(const Map &saved)
{
...
void MapCanvasCore::Diff::maybeAsyncUpdate(const Map &saved, const Map &current)
{
    auto &diff = *this;
...
void MapCanvasCore::paintDifferences()
{
    auto &diff = m_diff;
...
void MapCanvasCore::paintSelections()
{
    paintSelectedRooms();
...
void MapCanvasCore::hostPaintGL()
{
...
void MapCanvasCore::paintSelectionArea()
{
    if (!hasSel1() || !hasSel2()) {
        return;
    }
